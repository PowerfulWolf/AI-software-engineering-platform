...
``RepositoryProfile`` URI and digest.
"""
    ...
    def validate_result(self) -> Self:
        if self.status is ProjectBaselineCompilationStatus.COMPILED:
            ...
            if (
                ...
                or self.compiled_spec.repository_profile_sha256 != self.repository_profile_sha256
            ):
            ...
            if any(
                conflict.repository_id != self.repository_id
                or conflict.repository_profile_sha256 != self.repository_profile_sha256
...
def _require_workspace_identity(
    ...
    if compilation.repository_id != workspace.repository_id:
        raise ProjectBaselineRecordCorruption(
