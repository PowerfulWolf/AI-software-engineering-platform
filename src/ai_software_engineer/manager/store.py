"""Immutable persistence for Manager stage checkpoints.
...
_PROJECT_ID_ADAPTER: Final[TypeAdapter[RepositoryId]] = TypeAdapter(RepositoryId)
...
class ProjectPreparationStore(Protocol):
    """Port exposed to Manager application services, never directly to an Agent."""
...
class FileProjectPreparationStore:
    ...
    def put(self, preparation: ProjectPreparation) -> ProjectPreparation:
        ...
        target = self._path(preparation.repository_id)
        if target.exists() or target.is_symlink():
            existing = self.get(preparation.repository_id)
        ...
        return self.get(preparation.repository_id)
    ...
    def get(self, repository_id: RepositoryId | str) -> ProjectPreparation:
        ...
        if target.is_symlink():
            raise ProjectPreparationPathError(
                f"ProjectPreparation record cannot be a symlink: {repository_id}"
            )
        if not target.is_file():
            if target.exists():
                raise ProjectPreparationPathError(
                    f"ProjectPreparation record path is not a file: {repository_id}"
                )
            raise ProjectPreparationNotFound(str(repository_id))
        try:
            ...
            if not isinstance(payload, dict) or set(payload) != _RECORD_KEYS:
                raise ProjectPreparationCorruption(
                    f"ProjectPreparation record has an invalid envelope: {repository_id}"
                )
            preparation_payload = payload.get("preparation")
            digest = payload.get("sha256")
            if not isinstance(preparation_payload, dict) or not isinstance(digest, str):
                raise ProjectPreparationCorruption(
                    f"ProjectPreparation record is incomplete: {repository_id}"
                )
            if digest != _digest(preparation_payload):
                raise ProjectPreparationCorruption(
                    f"ProjectPreparation record digest mismatch: {repository_id}"
                )
            ...
            raise ProjectPreparationCorruption(
                f"cannot decode ProjectPreparation record: {repository_id}"
            ) from error
        if preparation.repository_id != repository_id:
            raise ProjectPreparationCorruption(
                f"ProjectPreparation filename identity mismatch: {repository_id}"
            )
        return preparation
    ...
    def _path(self, repository_id: RepositoryId | str) -> Path:
        try:
            validated_id = _PROJECT_ID_ADAPTER.validate_python(repository_id)
        except ValidationError as error:
            raise ProjectPreparationNotFound(str(repository_id)) from error
        if (
...
def _validate_preparation(
    ...
    try:
        ...
        raise error_type(
            f"ProjectPreparation digest mismatch: {preparation.repository_id}"
        ) from error
