"""Manager Agent facade for task-free project preparation.

The Agent supplies only an absolute repository root. Organization identity, sidecar
...
from ai_software_engineer.runtime_workspace import RuntimeWorkspaceBinder, TeamWorkforceWorkspace
...
class ManagerSkillError(RuntimeError):
    """Base error for fail-closed Manager Skill behavior."""


class ProjectPreparationDrift(ManagerSkillError):
    """Raised when current repository facts disagree with a durable preparation."""


class ProjectPreparationCompositionError(ManagerSkillError):
    """Raised when organization, target, and sidecar roots cannot compose safely."""


class ProjectBaselineRecordError(ManagerSkillError):
    """Raised when a baseline recorder returns a different compilation identity."""


class ProductContextNotReady(ManagerSkillError):
    """Raised when Product Agent context is requested before preparation."""


class PrepareProjectStatus(StrEnum):
    """Exclusive outcomes visible to the Manager Agent."""
...
class PrepareProjectResult(DomainModel):
    ...
    def validate_exclusive_result(self) -> Self:
        if self.status is PrepareProjectStatus.PREPARED:
            ...
            if self.preparation.repository_id != self.repository_id:
            ...
            if any(conflict.repository_id != self.repository_id for conflict in self.conflicts):
                raise ValueError("conflicts belong to another repository")
...
class ManagerSkill(Protocol):
    """Agent-visible, policy-bound Manager capability."""
...
class ProjectRuleProvider(Protocol):
    """Resolve explicit rules authorized for the observed repository profile."""
...
class ManagerSkillService:
    ...
    def __init__(
        ...
        *,
        ...
        registry: RepositoryWorkspaceRegistry,
    ...
    def prepare_project(self, request: PrepareProjectRequest) -> PrepareProjectResult:
        ...
        workspace = self._registry.register(request.repository_root)
        store = self._preparation_store_factory(workspace.directory("policy"))
        existing = store.find(workspace.repository_id)
        ...
        if self._versioned_preparations and (
            existing is None or existing.repository_profile_sha256 != profile.profile_sha256
        ):
        ...
        if recorded.status is ProjectBaselineCompilationStatus.CONFLICT:
            if existing is not None:
                raise ProjectPreparationDrift(
                    "current repository baseline conflicts with its durable preparation"
                )
        ...
        candidate = ProjectPreparation.create(
            team_id=self._organization.team_id,
            project_id=binding.project_id,
            project_manifest_sha256=binding.project_manifest_sha256,
            repository_id=workspace.repository_id,
            repository_root=str(workspace.repository_root),
            repository_workspace_root=str(workspace.root),
            team_root=str(self._organization.root),
        ...
        if existing is not None and existing != candidate:
            raise ProjectPreparationDrift(
                "current repository facts do not match the durable preparation"
            )
    ...
    def advance_stage(self, request: StageAdvanceRequest) -> StageAdvanceAuthorization:
        ...
        current = self.prepare_project(
            PrepareProjectRequest(repository_root=preparation.repository_root)
        )
        if (
            ...
            raise ProjectPreparationDrift(
                "stage advancement checkpoint does not match current repository facts"
            )
    ...
    def require_product_context(self, result: PrepareProjectResult) -> ProjectPreparation:
        ...
        current = self.prepare_project(
            PrepareProjectRequest(repository_root=supplied.repository_root)
        )
        if (
            current.status is not PrepareProjectStatus.PREPARED
            or current.preparation is None
            ...
            raise ProjectPreparationDrift(
                "Product Agent checkpoint does not match current repository facts"
            )
...
def _require_aware(value: datetime) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise ManagerSkillError("Manager clock must be timezone-aware")
