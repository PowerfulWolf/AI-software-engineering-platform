"""Concrete Manager backend for the Team-owned production team."""
...
class ProductionProjectDeliveryBackend:
    ...
    def __init__(
        ...
        *,
        ...
        project_rule_provider: ProjectRuleProvider | None = None,
        ...
        self._preparer = ManagerSkillService(
            ...
            rule_provider=project_rule_provider or _NoProjectRules(),
    ...
    def prepared_context(self, preparation: PrepareProjectResult) -> tuple[ContextSource, ...]:
        ...
        return (
            ...
            ContextSource(
                ...
                uri=f"baseline://{facts.profile.repository_id}",
        ...
        def execute() -> DesignerServiceResult:
            ...
            return service.run(
                RunDesignerCommand(
                    ...
                    repository_profile=facts.profile,
    ...
    def _commit_dispatch(self, checkpoint: ProjectDeliveryCheckpoint) -> DispatchCommitRecord:
        ...
        workforce_store = FileTeamWorkforceStore(self._organization)
    ...
    def run_prepared_allocation(
        ...
        *,
        ...
        facts = self._facts(preparation)
        if dispatch.repository_id != facts.workspace.repository_id:
        ...
        runtime_config = RuntimeConfig(
            ...
            context_sources=(
                ...
                *extra_context,
                ...
                ContextSource(
                    source_id="project.baseline",
                    uri=f"baseline://{facts.baseline.repository_id}/{facts.baseline.baseline_sha256}",
                    content=json.dumps(
    ...
    def _workforce(
        ...
        return production_team_roster(self._config)
...
def _task_commands(profile: RepositoryProfile) -> tuple[str, ...]:
    commands = {
        "git status",
        "git diff",
        "git rev-parse",
...
def _clean_git_head(repository_root: Path) -> str:
    ...
    if Path(git("rev-parse", "--show-toplevel")).resolve() != repository_root.resolve():
...
def _runtime_paths(workspace: RepositoryWorkspace) -> RuntimePaths:
    return RuntimePaths(
