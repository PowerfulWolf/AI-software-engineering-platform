"""Atomic Manager commit-dispatch boundary.
...
class DispatchWorkforceSnapshot(DomainModel):
    """Versioned current facts read from the Team-owned allocation authority."""
    ...
    def validate_snapshot(self) -> Self:
        if (
            self.work_item.task_id != self.task_id
            or self.work_item.repository_id != self.repository_id
        ):
...
class DispatchCommitRecord(DomainModel):
    ...
    def validate_record(self) -> Self:
        ...
        if self.task.metadata.get("repository_id") != self.repository_id:
        ...
        if any(phase.assignment.repository_id != self.repository_id for phase in self.phases):
...
class RecoveryDispatchRecord(DomainModel):
    ...
    def validate_record(self) -> Self:
        if (
            ...
            or self.task.metadata.get("repository_id") != self.repository_id
        ...
        if any(
            p.assignment.task_id != self.task_id or p.assignment.repository_id != self.repository_id
...
class ContinuationDispatchRecord(DomainModel):
    ...
    def validate_record(self) -> Self:
        ...
        if any(
            ...
            or phase.assignment.repository_id != self.repository_id
...
class ManagerDispatchService:
    ...
    def commit_dispatch(self, request: CommitDispatchRequest) -> DispatchCommitRecord:
        ...
        snapshot = self._authority.current_snapshot(
            repository_id=request.preparation.repository_id,
            task_id=task.id,
        )
    ...
    def _validate_planner_handoff(self, request: CommitDispatchRequest) -> None:
        ...
        if (
            ...
            or run.repository_id != request.preparation.repository_id
            ...
            or checkpoint.repository_id != request.preparation.repository_id
    ...
    def _validate_preview(
        ...
        if (
            preview.repository_id != request.preparation.repository_id
        ...
        if (
            ...
            or snapshot.work_item.repository_id != request.preparation.repository_id
