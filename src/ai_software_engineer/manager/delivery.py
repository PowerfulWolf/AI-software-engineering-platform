"""Unified Manager entry point for one resumable serial delivery."""
...
class UnifiedProjectEntryError(RuntimeError):
    """Base error exposed by the Manager delivery facade."""
...
class StartProjectDelivery(DomainModel):
    ...
    additional_repository_roots: tuple[NonEmptyStr, ...] = ()
...
class ProjectDeliveryBackend(Protocol):
    """Native services composed behind the Manager Agent Skill facade.
...
class ProjectDeliveryCheckpointCatalog:
    ...
    def for_project(self, repository_id: RepositoryId | str) -> FileProjectDeliveryCheckpointStore:
        sidecar = self._root / str(repository_id)
        if sidecar.is_symlink() or not sidecar.is_dir():
            raise DeliveryCatalogError(f"project sidecar is missing: {repository_id}")
        state = sidecar / "state"
        if state.is_symlink() or not state.is_dir():
            raise DeliveryCatalogError(f"project state root is missing: {repository_id}")
    ...
    *,
    ...
    if source_indices:
        ...
        successor_cursor = bool(tail) and all(
            checkpoint.repository_id == current.repository_id
            and checkpoint.repository_root == current.repository_root
    ...
    return (
        ...
        and current.repository_id == plan.scope.repository_id
...
class UnifiedProjectEntryService:
    ...
    def start(self, command: StartProjectDelivery) -> ProjectDeliveryResult:
        ...
        delivery_id = _delivery_id(
            command.repository_root, command.requirement, namespace=self._delivery_namespace
        )
        preparation = self._backend.prepare(command.repository_root)
        store = self._catalog.for_project(preparation.repository_id)
        try:
            current = store.current(delivery_id)
        except ProjectDeliveryCheckpointNotFound:
            current = None
        if current is not None:
            if current.repository_root != str(Path(command.repository_root).resolve()):
        ...
        if first is None:
            try:
                intake = store.get_intake(delivery_id)
                self._require_same_intake(intake, command)
                if intake.repository_id != preparation.repository_id:
                ...
                intake = store.put_intake(
                    ProjectDeliveryIntake.create(
                        ...
                        repository_root=str(Path(command.repository_root).resolve()),
                        title=command.title,
    ...
    def resume(self, command: ResumeProjectDelivery) -> ProjectDeliveryResult:
        ...
        if current.stage in {DeliveryStage.PREPARING, DeliveryStage.PRODUCT_DISCOVERY}:
            ...
            preparation = self._backend.prepare(intake.repository_root)
            if preparation.repository_id != current.repository_id:
    ...
    def begin_recovery(
        ...
        *,
        ...
        if (
            ...
            or dispatch.repository_id != plan.source.scope.repository_id
            or dispatch.task.repository != plan.source.scope.repository_root
    ...
    def _require_same_intake(intake: ProjectDeliveryIntake, command: StartProjectDelivery) -> None:
        if (
            intake.repository_root != str(Path(command.repository_root).resolve())
...
def _delivery_id(
    repository_root: str, requirement: str, *, namespace: str | None = None
) -> DeliveryId:
    identity = {"repository_root": str(Path(repository_root).resolve()), "requirement": requirement}
