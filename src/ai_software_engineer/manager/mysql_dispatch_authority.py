...
class MySqlDispatchAuthority:
    ...
    def seed_snapshot(self, snapshot: DispatchWorkforceSnapshot) -> DispatchWorkforceSnapshot:
        ...
        with (
            ...
            cursor.execute(
                """
                SELECT repository_id, task_id, payload_json, snapshot_sha256
                FROM dispatch_workforce_snapshots
                WHERE repository_id = %s AND task_id = %s FOR UPDATE
                """,
                (snapshot.repository_id, snapshot.task_id),
            )
            ...
            cursor.execute(
                ...
                INSERT INTO dispatch_workforce_snapshots
                    (repository_id, task_id, payload_json, snapshot_sha256)
                VALUES (%s, %s, %s, %s)
                """,
                (
    ...
    def current_snapshot(
        ...
        *,
        ...
        with (
            ...
            return self._current_snapshot(connection, repository_id, task_id)
    ...
    def commit_if_current(
        ...
        *,
        ...
        with (
            ...
            cursor.execute(
                """
                SELECT id, repository_id, task_id, payload_json, dispatch_sha256
            ...
            snapshot = self._current_snapshot(connection, record.repository_id, record.task_id)
            ...
            try:
                cursor.execute(
                    ...
                    INSERT INTO dispatch_commits
                        (id, repository_id, task_id, payload_json, dispatch_sha256)
                    VALUES (%s, %s, %s, %s, %s)
                    """,
                    (
                        record.id,
            ...
            self._current_snapshot(connection, record.repository_id, record.task_id)
            return record
    ...
    def commit_recovery(
        ...
        *,
        ...
        with (
            ...
            snapshot = self._current_snapshot(connection, repository_id, task_id)
            ...
            self._current_snapshot(connection, repository_id, task_id)
            return record
        ...
        *,
        ...
        with (
            ...
            if row is not None:
                ...
                return record
            validate_current(None)
            snapshot = self._current_snapshot(connection, repository_id, source_task_id)
            record = build(snapshot)
    ...
    def _current_snapshot(
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                SELECT repository_id, task_id, payload_json, snapshot_sha256
                FROM dispatch_workforce_snapshots
                WHERE repository_id = %s AND task_id = %s
                """,
            ...
            if row is None:
                raise DispatchAuthorityConflict(
                    f"workforce snapshot is not registered for {repository_id}/{task_id}"
                )
            base = _decode_snapshot(row)
            if base.repository_id != repository_id or base.task_id != task_id:
    ...
    def _validate_handoff(self, record: DispatchCommitRecord) -> None:
        ...
        if (
            ...
            or current.request.repository_id != record.repository_id
            ...
            or run.repository_id != record.repository_id
...
def _decode_snapshot(row: Mapping[str, object]) -> DispatchWorkforceSnapshot:
    try:
        ...
        snapshot.validate_integrity()
        if (
            _text(row, "repository_id") != snapshot.repository_id
...
def _decode_allocation(row: Mapping[str, object]) -> DeliveryAllocation:
    try:
        ...
        if (
            ...
            or _text(row, "repository_id") != record.repository_id
