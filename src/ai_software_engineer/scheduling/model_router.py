...
from ai_software_engineer.domain.enums import BrainTier, ModelRouteReason, TeamRole
...
def _route_name(
    ...
    connection = f":{connection_mode}" if connection_mode is not None else ""
    return f"{provider}/{model}{suffix}{kind}{connection}"
...
class ModelRouter:
    ...
    def route(
        ...
        *,
        ...
        if TeamRole(demand.role.value) not in agent.eligible_roles:
            return self._reject(
                demand,
                agent,
                now,
        ...
        selection = ModelSelection(
            ...
            connection_mode=selected.connection_mode,
            tier=selected.tier,
