...
``RepositoryProfile`` deliberately describes observations rather than interpreting
...
from ai_software_engineer.repository_workspace import repository_id_for_root
...
class RepositoryProfileError(RuntimeError):
    """Base error for fail-closed profile discovery."""


class RepositoryProfileRootNotFound(RepositoryProfileError):
    """Raised when the requested repository root is not an existing directory."""


class RepositoryProfilePathEscape(RepositoryProfileError):
    """Raised when a discovered symlink would escape the repository root."""


class RepositoryProfileReadError(RepositoryProfileError):
    """Raised when a required project fact cannot be read consistently."""


class RepositoryProfileEncodingError(RepositoryProfileError):
    """Raised when a native rule source is not valid UTF-8."""


class RepositoryProfileMetadataError(RepositoryProfileError):
...
class VcsKind(StrEnum):
    """Version-control marker observed at the repository root."""
...
class RepositoryProfile(DomainModel):
    """Deterministic facts about one repository root and its native guidance."""

    kind: Literal["repository_profile"] = "repository_profile"
    schema_version: Literal["v0.1"] = "v0.1"
    repository_id: RepositoryId
    repository_root_uri: NonEmptyStr
    ...
    def validate_profile(self) -> Self:
        if self.repository_root_uri != f"project://{self.repository_id}":
            raise ValueError("repository_root_uri must be derived from repository_id")
        ensure_unique((fact.language for fact in self.languages), "RepositoryProfile languages")
        ensure_unique(
            (fact.system for fact in self.build_systems),
            "RepositoryProfile build systems",
        )
        ensure_unique(
            (source.uri for source in self.native_rules),
            "RepositoryProfile native rule URIs",
        )
        for source in self.native_rules:
            expected_uri = f"project://{self.repository_id}/{source.relative_path}"
            if source.uri != expected_uri:
                raise ValueError("native rule URI must match repository_id and relative_path")
    ...
    def discover(
        ...
        *,
        ...
        """Read a repository root and return a typed, deterministic profile.
        ...
        selected_repository_id = _select_repository_id(root, repository_id)
        ...
        native_rules = _discover_native_rules(root, files, selected_repository_id)
        ...
        base = {
            ...
            "repository_root_uri": f"project://{selected_repository_id}",
    ...
    def validate_integrity(self) -> None:
        ...
        if self.profile_sha256 != _profile_digest(self):
            raise RepositoryProfileMetadataError(
                "RepositoryProfile profile_sha256 does not match content"
            )


def discover_repository_profile(
    repository_root: str | Path,
    *,
    repository_id: RepositoryId | str | None = None,
    observed_at: datetime | None = None,
...
def profile_digest(profile: RepositoryProfile) -> Sha256:
...
def _canonical_root(repository_root: str | Path) -> Path:
    candidate = Path(repository_root).expanduser().resolve(strict=False)
    if not candidate.is_dir():
        raise RepositoryProfileRootNotFound(str(candidate))
    return candidate


def _select_repository_id(root: Path, repository_id: RepositoryId | str | None) -> RepositoryId:
    if repository_id is None:
        return repository_id_for_root(root)
    try:
        from pydantic import TypeAdapter, ValidationError

        return TypeAdapter(RepositoryId).validate_python(repository_id)
    except ValidationError as error:
        raise RepositoryProfileMetadataError(f"invalid project ID: {repository_id!r}") from error
...
def _iter_project_files(root: Path) -> Iterator[Path]:
    ...
    while pending:
        ...
        try:
            ...
            raise RepositoryProfileReadError(
...
def _ensure_contained(root: Path, path: Path) -> None:
    try:
        ...
        raise RepositoryProfilePathEscape(
            f"cannot resolve discovered symlink: {path.relative_to(root).as_posix()}"
        ) from error
    if not resolved.is_relative_to(root):
        raise RepositoryProfilePathEscape(
            f"discovered symlink escapes repository root: {path.relative_to(root).as_posix()}"
        )
...
def _discover_vcs(root: Path) -> VcsInfo:
    ...
    if len(markers) > 1:
        raise RepositoryProfileMetadataError("multiple VCS markers found at repository root")
    if not markers:
...
def _read_git_info(root: Path, git_dir: Path, marker: str) -> VcsInfo:
    ...
    try:
        ...
        raise RepositoryProfileReadError("cannot read Git HEAD metadata") from error
    if not head:
        raise RepositoryProfileMetadataError("Git HEAD metadata is empty")
    if head.startswith("ref: "):
        ref = head.removeprefix("ref: ").strip()
        if not _REF_PATTERN.fullmatch(ref):
            raise RepositoryProfileMetadataError("Git HEAD contains an invalid ref")
        ...
        try:
            ...
            raise RepositoryProfileReadError("cannot read Git ref metadata") from error
        if revision is not None and not _REVISION_PATTERN.fullmatch(revision):
            raise RepositoryProfileMetadataError("Git ref contains an invalid revision")
        return VcsInfo(kind=VcsKind.GIT, marker=marker, revision=revision, ref=ref)
    if not _REVISION_PATTERN.fullmatch(head):
        raise RepositoryProfileMetadataError("Git HEAD contains neither a valid ref nor revision")
...
def _discover_native_rules(
    ...
    for relative in sorted(kinds_by_path):
        ...
        try:
            ...
            raise RepositoryProfileReadError(
                f"cannot read native rule source: {relative}"
            ) from error
        try:
            content.decode("utf-8")
        except UnicodeDecodeError as error:
            raise RepositoryProfileEncodingError(
...
def _validate_revision(revision: str) -> None:
    if not _REVISION_PATTERN.fullmatch(revision):
        raise RepositoryProfileMetadataError("revision must be a 40-64 character lowercase hex SHA")


def _profile_digest(profile: RepositoryProfile) -> Sha256:
...
__all__ = [
    ...
    "RelativePath",
    "RepositoryProfile",
    "RepositoryProfileEncodingError",
    "RepositoryProfileError",
    "RepositoryProfileMetadataError",
    "RepositoryProfilePathEscape",
    "RepositoryProfileReadError",
    "RepositoryProfileRootNotFound",
    "Sha256",
    "VcsInfo",
    "VcsKind",
