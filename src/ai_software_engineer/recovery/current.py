...
class NativeRecoveryFactsVerifier:
    ...
    def _inspect(self, plan: RecoveryPlan) -> NativeRecoveryFacts:
        ...
        sidecar = company.root / "projects" / source.scope.project_id
        target = _preparation(sidecar, source.scope.project_id, plan.target_preparation_sha256)
        ...
        profile_path = _record_read_path(
            sidecar / "profile", PROJECT_PROFILE_NAME, target.project_profile_sha256
        )
        _reject_symlinks(profile_path)
        profile = ProjectProfile.model_validate_json(_read_regular(profile_path, 8_000_000))
        profile.validate_integrity()
        if (
            profile.profile_sha256 != target.project_profile_sha256
        ...
        if (
            ...
            or binding.project_profile_sha256 != profile.profile_sha256
            or binding.project_root != target.project_root
            or binding.project_workspace_root != str(sidecar)
            or binding.organization_root != target.organization_root
            or binding.organization_id != target.organization_id
            or binding.project_id != target.project_id
        ):
            raise ValueError("target runtime binding mismatch")
        # Establish scope before the binding validator follows any embedded path.
        _reject_symlinks(Path(target.organization_root))
        _read_regular(Path(target.organization_root) / "organization.json", 1_000_000)
        binding.validate_environment()
        knowledge = company.knowledge_sources(config.company_knowledge_paths)
        compiled = ProjectBaselineCompiler().compile(
            profile, production_rules(company, knowledge), compiled_at=target.prepared_at
        )
        ...
        if (
            ...
            raise ValueError("current rules or company knowledge changed")
        manager = GitWorktreeManager(
            target.project_root, Path(config.platform_root) / "worktrees" / target.project_id
        )
