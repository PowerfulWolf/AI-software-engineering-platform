...
def _expand_platform_root(value: str) -> str:
    """Expand the only supported home-relative form before path validation."""
    ...
    suffix = value[2:]
    if suffix.startswith("/") or any(part == ".." for part in Path(suffix).parts):
        raise ValueError("platform_root must be an absolute safe path")
...
class ProductionConfig(DomainModel):
    ...
    def expand_home_relative_platform_root(cls, value: object) -> object:
        ...
        return _expand_platform_root(value) if isinstance(value, str) else value
