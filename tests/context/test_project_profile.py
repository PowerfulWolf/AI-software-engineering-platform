...
def test_marker_heavy_profile_is_compact_without_losing_rules(tmp_path: Path) -> None:
    ...
    profile = ProjectProfile.discover(tmp_path)
    original = profile.to_wire()
    source = project_profile_context(profile)
    payload = json.loads(source.content or "")
    assert payload["kind"] == "project_profile_context"
    ...
    assert source == project_profile_context(profile)
