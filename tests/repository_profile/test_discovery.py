"""Read-only, deterministic RepositoryProfile discovery contract tests."""
...
from ai_software_engineer.repository_profile import (
    BuildSystem,
    ProjectLanguage,
    RepositoryProfile,
...
def _validator() -> Draft202012Validator:
    schema_path = Path(__file__).parents[2] / "schemas" / "repository-profile.schema.json"
...
def test_empty_project_is_unknown_deterministic_and_read_only(tmp_path: Path) -> None:
    ...
    first = discover_repository_profile(project, observed_at=OBSERVED)
    replay = discover_repository_profile(project, observed_at=OBSERVED + timedelta(hours=1))
...
def test_native_rule_sources_use_project_uris_hashes_and_schema(tmp_path: Path) -> None:
    ...
    profile = discover_repository_profile(
        project,
        repository_id="repository_rules_001",
    ...
    for source in profile.native_rules:
        assert source.uri == f"project://repository_rules_001/{source.relative_path}"
...
def test_git_head_ref_and_expected_revision_are_verified(tmp_path: Path) -> None:
    ...
    with pytest.raises(RepositoryProfileMetadataError, match="does not match"):
...
def test_discovery_rejects_symlink_escape(tmp_path: Path) -> None:
    ...
    with pytest.raises(RepositoryProfilePathEscape):
        discover_repository_profile(project, observed_at=OBSERVED)
...
def test_native_rule_invalid_utf8_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(RepositoryProfileEncodingError):
        discover_repository_profile(project, observed_at=OBSERVED)
...
def test_profile_model_rejects_cross_project_native_rule_uri(tmp_path: Path) -> None:
    ...
    payload = discover_repository_profile(
        project,
        repository_id="repository_expected_001",
        observed_at=OBSERVED,
    ).to_wire()
    native_rules = payload["native_rules"]
    assert isinstance(native_rules, list)
    first_rule = cast(dict[str, object], native_rules[0])
    first_rule["uri"] = "project://repository_wrong_001/README.md"
