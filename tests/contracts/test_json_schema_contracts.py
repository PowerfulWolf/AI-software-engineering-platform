...
def test_console_operation_states_satisfy_the_canonical_schema(tmp_path: Path) -> None:
    ...
    recheck_design = ConsoleOperation.queued(
        team_id="team_test",
        idempotency_key="browser-action-recheck-design-0001",
        intent=RecheckDesignIntent(
            project_id="project_test",
            delivery_id="delivery_multi_" + "a" * 40,
            expected_checkpoint_sha256="2" * 64,
        ),
    ...
    _assert_valid(recheck_design.to_wire(), "console-operation.schema.json")
