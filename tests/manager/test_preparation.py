"""Manager prepare_project Skill orchestration tests."""
...
from ai_software_engineer.manager.stages import ProjectStage, StageAdvanceRequest
from ai_software_engineer.manager.store import (
    FileProjectPreparationStore,
    ProjectPreparationCorruption,
)
from ai_software_engineer.repository_profile import ProjectLanguage, RepositoryProfile
...
def service(
    ...
    *,
    ...
    project = team.project_registry().register(
        project_id="project_preparation_001",
...
def test_only_project_directory_prepares_language_agnostic_project_without_pollution(
    ...
    result = skill.prepare_project(PrepareProjectRequest(repository_root=str(project.resolve())))

    assert result.status is PrepareProjectStatus.PREPARED
    preparation = skill.require_product_context(result)
    assert preparation.repository_root == str(project.resolve())
    assert preparation.team_id == "team_preparation_001"
    ...
    assert Path(preparation.repository_workspace_root).is_relative_to(
        tmp_path / "platform" / "projects" / "project_preparation_001" / "repositories"
    )
...
def test_exact_replay_returns_first_preparation_and_timestamp(tmp_path: Path) -> None:
    ...
    policy_root = Path(replay.preparation.repository_workspace_root) / "policy"
...
def test_project_baseline_conflict_is_durable_and_blocks_product_context(tmp_path: Path) -> None:
    ...
    sidecar = next(
        (tmp_path / "platform" / "projects" / "project_preparation_001" / "repositories").iterdir()
    )
...
def test_corrupted_preparation_fails_before_product_context_can_be_returned(
    ...
    record = next(
        (Path(result.preparation.repository_workspace_root) / "policy").glob(
...
def test_product_context_gate_rejects_self_consistent_forged_checkpoint(
    ...
    forged = type(trusted).create(
        ...
        project_manifest_sha256=trusted.project_manifest_sha256,
        repository_id=trusted.repository_id,
        repository_root=trusted.repository_root,
        repository_workspace_root=trusted.repository_workspace_root,
        team_root=trusted.team_root,
        repository_profile_sha256=trusted.repository_profile_sha256,
    ...
    with pytest.raises(ProjectPreparationDrift, match="current repository facts"):
...
def test_service_authorizes_product_context_only_from_prepared_checkpoint(
    ...
    assert authorization.repository_id == preparation.repository_id
...
def test_request_rejects_relative_paths_unknown_fields_and_naive_clock(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="absolute"):
        PrepareProjectRequest(repository_root="relative/project")
    with pytest.raises(ValidationError, match="extra"):
        PrepareProjectRequest.model_validate(
            {"repository_root": str(tmp_path.resolve()), "team_root": "/ambient"}
        )
