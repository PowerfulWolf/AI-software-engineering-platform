"""Manager Agent-visible Skill payloads match their wire Schema."""
...
from tests.manager.test_preparation import service
...
def _validator() -> Draft202012Validator:
    ...
    manager_schema: dict[str, object] | None = None
    for path in SCHEMA_DIR.glob("*.schema.json"):
        payload = cast(dict[str, object], json.loads(path.read_text(encoding="utf-8")))
        schemas.append((cast(str, payload["$id"]), Resource.from_contents(payload)))
        if path.name == "agent-skill-manager.schema.json":
            manager_schema = payload
    assert manager_schema is not None
...
def test_skill_schema_rejects_extra_authority_and_wrong_digest_cardinality(
    ...
    project = tmp_path / "target"
    project.mkdir()
    request = PrepareProjectRequest(repository_root=str(project.resolve())).to_wire()
    request["team_root"] = "/ambient-authority"
    assert _errors(request)

    control_path = PrepareProjectRequest(repository_root=str(project.resolve())).to_wire()
    control_path["repository_root"] = "/tmp/project\nforged"
    ...
    invalid_authorization: WirePayload = {
        ...
        "repository_id": "repository_delivery_001",
