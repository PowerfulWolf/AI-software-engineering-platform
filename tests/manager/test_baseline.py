...
def profile(tmp_path: Path) -> RepositoryProfile:
    ...
    return RepositoryProfile.discover(
        project,
        repository_id="repository_prepare_001",
        observed_at=NOW,
    )
...
def test_compiles_stable_task_free_baseline_with_opaque_native_sources(
    ...
    first = compiler.compile(repository_profile, rules, compiled_at=NOW)
    ...
    assert first.compiled_spec.repository_profile_sha256 == repository_profile.profile_sha256
...
def test_non_overlapping_scopes_with_different_values_do_not_conflict(tmp_path: Path) -> None:
    ...
    result = ProjectBaselineCompiler().compile(
        ...
        (
            ...
            project_rule(repository_profile, scopes=("language:java",)),
        ),
        compiled_at=NOW,
    )
...
def test_structured_project_sources_are_removed_from_opaque_sources(tmp_path: Path) -> None:
    ...
    repository_profile = RepositoryProfile.discover(
        project,
        repository_id="repository_prepare_001",
        observed_at=NOW,
    )
    structured = project_rule(repository_profile)
    ...
    assert opaque_uris == {
        source.uri
        for source in repository_profile.native_rules
        if source.uri != structured.source_uri
    }
...
def test_missing_hard_safety_and_task_rule_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(HardPolicyMissing):
        compiler.compile(repository_profile, (engineering_rule(),), compiled_at=NOW)
    ...
    with pytest.raises(TaskScopedRuleRejected, match="before a Task exists"):
        compiler.compile(repository_profile, (hard_rule(), task_rule), compiled_at=NOW)


def test_project_rule_must_bind_exact_profile_source(tmp_path: Path) -> None:
    repository_profile = profile(tmp_path)
    changed = project_rule(repository_profile).model_copy(update={"source_sha256": "f" * 64})
...
def test_baseline_is_frozen_strict_and_integrity_checked(tmp_path: Path) -> None:
    ...
    changed = baseline.model_copy(update={"repository_profile_sha256": "f" * 64})
...
def test_file_store_is_append_once_and_exact_replay_returns_first_record(
    ...
    repository_profile = profile(tmp_path)
    workspace = RepositoryWorkspaceRegistry(
        tmp_path / "sidecars",
        project_id="project_baseline_001",
    ...
    compiler = ProjectBaselineCompiler()
    first = compiler.compile(repository_profile, (hard_rule(),), compiled_at=NOW)
    replay = compiler.compile(
        repository_profile,
        (hard_rule(),),
...
def test_file_store_separates_conflicts_and_rejects_tampered_envelope(tmp_path: Path) -> None:
    ...
    replay = ProjectBaselineCompiler().compile(
        repository_profile,
        tuple(reversed((hard_rule(), engineering_rule(), project_rule(repository_profile)))),
        compiled_at=NOW + timedelta(hours=1),
    )
