...
- `ModelPolicy`：eligible provider/model/reasoning/route kind/connection mode routes、默认 BrainTier 和完整 risk floors；
- `ModelSelection`：一次 Run 的 provider/model/reasoning effort/route kind/connection mode/tier/policy version 与选择理由；
...
model 的 policy/version/provider/model/reasoning-effort/route-kind/connection-mode/tier 语义也必须相同，且不能低于 ExecutionPlan 的 minimum
...
`ProjectRuleProvider.sources_for()` 精确授权后才能进入 baseline；Repository 原生规范仍使用
RepositoryProfile provenance。三者对同一字段给出不同值时生成冲突并等待人工，不按优先级猜测。
知识或 Spec 快照变化只重建相关 Project runtime，不要求进程重启，但旧 preparation 不能静默沿用。

`ProjectLearningStore.collect()` 只读取已经持久化且完整性有效的 QA FAIL / Review REJECT
Artifact，按 finding 生成确定性、证据化 proposal 与 recurrence count。`decide()` 要求 exact
